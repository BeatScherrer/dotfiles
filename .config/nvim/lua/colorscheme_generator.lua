...
--       That indecates whether to load the colorscheme use currently
--       loaded highlights
--
--It will create a basic vim colorscheme project in current
--directory named color_name.nvim
--
--If you want to remove any chance of plugin high;ight groups start neovim with nvim -u NONE to run this script

local bit = require("bit")

-- project_dir where plugin a created . By default current dir
...
local function create_project(color_name)
	...
	vim.fn.mkdir(table.concat({ project_base_dir, project_name, "colors" }, path_sep), "p")
	vim.fn.mkdir(table.concat({ project_base_dir, project_name, "lua", color_name }, path_sep), "p")
end
...
local function write_boilerplate(color_name)
	...
	-- open files
	local colorscheme_loader = io.open(
		table.concat({ project_base_dir, project_name, "colors", color_name .. ".vim" }, path_sep),
		"w"
	)
	local colorscheme_init = io.open(
		table.concat({ project_base_dir, project_name, "lua", color_name, "init.lua" }, path_sep),
		"w"
	)
	local colorscheme_utils = io.open(
		table.concat({ project_base_dir, project_name, "lua", color_name, "utils.lua" }, path_sep),
		"w"
	)
	local colorscheme_credits = io.open(table.concat({ project_base_dir, project_name, "CREDITS.md" }, path_sep), "w")
	local colorscheme_readme = io.open(table.concat({ project_base_dir, project_name, "README.md" }, path_sep), "w")
...
local function write_color_palette(palette, color_name)
	...
	-- open files
	local colorscheme_palette = io.open(
		table.concat({ project_base_dir, project_name, "lua", color_name, "palette.lua" }, path_sep),
		"w"
	)
...
local function write_color_defs(color_defs, inverse_palette, colorscheme_name)
	...
	-- open files
	local colorscheme_defs = io.open(
		table.concat({ project_base_dir, project_name, "lua", colorscheme_name, "definitions.lua" }, path_sep),
		"w"
	)
...
local highlights = {
	...
	for _, highlight in pairs(color_defs) do
		...
		if color.link then
			colorscheme_defs:write(string.format([[  %-10s = {link = '%s'}]], color_name, color.link) .. ",\n")
		else
			local command = string.format([[  %-10s = { ]], color_name)
			if color.foreground ~= nil then
				command = table.concat({ command, "fg = palette.", inverse_palette[color.foreground], ", " })
				color.foreground = nil
			end
			if color.background ~= nil then
				command = table.concat({ command, "bg = palette.", inverse_palette[color.background], ", " })
				color.background = nil
			end
			command = command .. string.gsub(string.match(vim.inspect(color), "{(.*)"), "\n", " ") .. ",\n"
...
local function add_to_palette(color_num, color_palette, color_count)
	...
	if not available then
		table.insert(color_palette, { color_name = "color" .. tostring(color_count), value = color_num })
		color_count = color_count + 1
	end
	return color_palette, color_count
end

-- Creats a color -> color_name lookup table from palette
...
-- genarates palettes and highlights table
...
---@param load boolean:
---     Indecates whether to load colorscheme or use current
---     highlight groups. When true current highlits are
---     cleared and colorscheme name color_name is loaded
---     with colorscheme command
local function create(color_name, load)
	if load and not load_colorscheme(color_name) then
		return
	end
